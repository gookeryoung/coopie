...
| 工具 | 用途 | 配置要点 |
|------|------|---------|
| **ruff** | lint + format | `line-length=120`，`target-version="{{ target_py }}"` |
| **pyrefly** | 类型检查 | `preset="strict"`，`python-version="{{ min_python_version }}"` |
| **pytest** | 测试 | `asyncio_default_fixture_loop_scope="function"`，marker `slow` |
| **coverage** | 覆盖率 | `branch=true`，`fail_under={{ coverage_fail_under }}`，`concurrency=["thread"]` |
...
- **最低 Python {{ min_python_version }}**：用 `from __future__ import annotations` 延迟注解求值；
  按版本用 `typing.List`({{ min_python_version }}) → 内置泛型(3.9) → `X | Y`(3.10) → `typing.override`(3.12)。
...
- **覆盖率 ≥ {{ coverage_fail_under }}%**（branch coverage），不得下降。
