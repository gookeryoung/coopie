...
要点：
- 模块顶部 `logger = logging.getLogger(__name__)`，`__name__` 自动形成层级（`{{ package_name }}.core.sub`）。
...
```python
"""{{ package_name }}.logging_config 模块."""
...
PROD_CONFIG: dict[str, Any] = {
    "version": 1, "disable_existing_loggers": False,
    "formatters": {
        "json": {"()": "{{ package_name }}.logging_config.JsonFormatter"},
        "simple": {"format": _SIMPLE_FMT},
    },
    "handlers": {
        ...
        "file": {"class": "logging.handlers.RotatingFileHandler", "level": "INFO",
                  "formatter": "json", "filename": "logs/{{ package_name }}.log",
...
def setup_logging(env: str = "dev", log_dir: Path | None = None) -> None:
    ...
    if env == "prod" and log_dir is not None:
        ...
        config["handlers"]["file"]["filename"] = str(log_dir / "{{ package_name }}.log")
...
def make_size_handler(path: Path, max_bytes: int = 10 * 1024 * 1024, backup_count: int = 5) -> logging.handlers.RotatingFileHandler:
    ...
    命名：{{ package_name }}.log / .1 / .2 / ... / .5（最旧，超出删除）
    """
...
def configure_loggers() -> None:
    ...
    层级：root=ERROR（兜底）→ {{ package_name }}=INFO（自研默认）→ 子模块按需放低；
    ...
    # 调试某模块时动态放低（不改 root）：logging.getLogger("{{ package_name }}.db").setLevel(logging.DEBUG)
...
```python
"""{{ package_name }}.cli 模块（CLI 日志集成片段）."""
