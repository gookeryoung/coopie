...
- 现有 `coopie` 的 `cli.py` 默认用 argparse；引入 Typer 时在 `pyproject.toml` 加 `typer>=0.12.0` 依赖。
...
- 嵌套层级 ≤ 2（`coopie db migrate`）；超过说明拆分不合理。
...
2. **颜色码污染重定向输出**：未检测终端直接输出 ANSI 码，`coopie > log.txt` 含乱码。用 `click.echo` 的 `color` 参数或 `click.style` 自动检测。
...
10. **进度条写入 stdout 干扰管道**：`coopie process | grep` 时进度条混入数据流。进度条用 stderr（`rich.Console(stderr=True)`），数据输出用 stdout。
