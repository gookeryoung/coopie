...
- [x] 模板 pyproject.toml gui 依赖按 Python 版本区分：
  - `PySide2>=5.15.2.1; python_version <= "3.10"`
  - `PySide6>=6.5.0; python_version >= "3.11"`
- [x] main.py 模板兼容 PySide2/PySide6（try/except import + exec 兼容写法）
- [x] copier.yml project_type=gui 的 help 文本更新（反映双绑定）
- [x] rule-12 规则更新（项目根 + 模板内），同时覆盖 PySide2 和 PySide6：
  - 标题改为 PySide2/PySide6
  - 工具链表增加 PySide6
  - 兼容性段说明按版本自动选择
  - 代码示例用兼容写法
  - 枚举差异（Qt.AlignCenter → Qt.AlignmentFlag.AlignCenter 等）
- [x] README 同步更新 project_type 说明
- [x] 验证：ruff + pyrefly + pytest 100% + copier update 无冲突
- [x] bump 版本（feat → minor 0.3.0 → 0.4.0）+ 迁移 _commit v0.3.0 → v0.4.0
